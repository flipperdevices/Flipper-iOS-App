...
struct SendArchivedItem: AppIntent {
    ...
    static var description: IntentDescription = .init("Use your flipper to send or emulate an item from your archive.", categoryName: "Archive")
    ...
    @Parameter(title: "Archived item", description: "An item archived on your Flipper.", requestValueDialog: "Pick an item to Emulate/Send")
    var archivedItem: ArchivedItemEntity

    func perform() async throws -> some ProvidesDialog {
        guard let item = await dependencies.archiveModel.items.first(where: { $0.id.description == archivedItem.id })
        else { return .result(dialog: "\(archivedItem.name) not found in your Flipper archive.") }

        guard await dependencies.device.status == .connected
        else { return .result(dialog: "Unable to connect to Flipper. Make sure Bluetooth is enabled both on this phone and on your Flipper.") }
